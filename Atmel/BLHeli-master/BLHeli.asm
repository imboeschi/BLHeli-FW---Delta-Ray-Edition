...
.ENDIF
.EQU	DEFAULT_PGM_MULTI_DEMAG_COMP 		= 3 	; 1=Disabled	2=Low		3=High
...
.EQU	DEFAULT_PGM_MULTI_BEEP_STRENGTH	= 80	; Beep strength
.EQU	DEFAULT_PGM_MULTI_BEACON_STRENGTH	= 200	; Beacon strength
...
.EQU	DEFAULT_PGM_ENABLE_PWM_INPUT	 	= 1 	; 1=Enabled 	0=Disabled
...
.EQU	RCP_MAX			= 	198	; This is maximum RC pulse length
...
.EQU	RCP_PWM_FREQ_15KHZ		= 	4		; RC pulse pwm frequency is 15kHz
...
.EQU 	Lvc_Pulse_Timer_Init_On  = 1718		; Count of Timer0 interrupts to generate Pvc pulse pwm signal (length of Pcv pulse pwm signal high)
.EQU 	Lvc_Pulse_Timer_Init_Off = 859	; Count of Timer0 interrupts to generate Pvc pulse pwm signal (length of Pcv pulse pwm signal low)
...
.EQU 	DR_RCP_MAX = 133
.EQU	DR_RCP_MIN = 20
.EQU	DR_RCP_CROSSOVER_L = 60
.EQU	DR_RCP_CROSSOVER_H = 109	
	;; 
	;; Throttle smoothing variables
	;;
ACC_NEW_RCP_L:			.BYTE 1
ACC_NEW_RCP_H:			.BYTE 1
ACC_NEW_RCP_COUNT:		.BYTE 1
.EQU	NEW_RCP_WINDOW = 128			; Must be a power of two between 2 - 128
...
_Eep_Pgm_Low_Voltage_Lim:	.DB	0x04 ; modified by C. Hegarty to enable low voltage limit in multi mode
...
t0_int_ppm_timeout_set:
	...
	brne	PC+2
	ldi	I_Temp2, 99
	sts	Throttle_Percentage, I_Temp2
	lds	I_Temp1, New_Rcp
...
rcp_int_fall:
	...
	;; Range checking - ensure maximum value is DR_RCP_MAX
...
throttle_rescale:	
	...
	;; Note: The Delta Ray never seems to give a throttle signal narrower than about 10 pulses so this approach doesn't cause any
	;; problems at low throttle levels
	cpi	I_Temp1, DR_RCP_MAX
	brlo	dr_continue
	ldi	I_Temp1, DR_RCP_MAX
dr_continue:
	...
	lsr	XL
	add	I_Temp1, XL	; Note this cannot overflow (maximum value is 133*1.5 = 199)
	lsr	I_Temp1		; divide by two
	sts	Throttle_Percentage, I_Temp1

	mov 	I_Temp1, I_Temp3
	cpi	I_Temp1, DR_RCP_CROSSOVER_L
...
mid_throttle:
	...
	rjmp	rcp_int_check_legal_range
high_throttle:	
	;; above DR_RCP_CROSSOVER_H, throttle transformation is subtract 6 and multiply by 2 then add 1
	;; Note maximum value in is DR_RCP_MAX (133) so this calculation cannot overflow
	subi	I_Temp1, 6
	lsl	I_Temp1		; multiply by two
	inc	I_Temp1
	
rcp_int_check_legal_range:
	;; Check that RC pulse is within legal range
	;; Probably not necessary but just for safety's sake
	cpi	I_Temp1, RCP_MAX
...
rcp_int_limited:
	...
	;; Throttle smoothing 
	;; 
	lds	I_Temp3, ACC_NEW_RCP_L
	lds	I_Temp4, ACC_NEW_RCP_H
	ldi	I_Temp2, 0
	add	I_Temp3, I_Temp1
	adc	I_Temp4, I_Temp2
	sts	ACC_NEW_RCP_L, I_Temp3
	sts	ACC_NEW_RCP_H, I_Temp4
	lds	I_Temp2, ACC_NEW_RCP_COUNT
	inc	I_Temp2
	cpi	I_Temp2, NEW_RCP_WINDOW
	brlo	acc_rcp_done
	;;
	;; Enough New_RCP samples collected to update
	;;
	lds	I_Temp3, ACC_NEW_RCP_L
	lds	I_Temp4, ACC_NEW_RCP_H
	ldi	I_Temp1, NEW_RCP_WINDOW
	lsr	I_Temp1
acc_rcp_rotate:	
	lsr	I_Temp4
	ror	I_Temp3
	...
	mov	I_Temp1, I_Temp3
	ldi	I_Temp2, 0
	sts	ACC_NEW_RCP_L, I_Temp2
	sts	ACC_NEW_RCP_H, I_Temp2	
	sts	New_Rcp, I_Temp1			; Store new pulse length
	sbr	Flags2, (1<<RCP_UPDATED)	 	; Set updated flag
acc_rcp_done:
#if defined(LVC_PULSE_ON)
...
.IF MODE == 2	; Multi
	...
	st	X+, Temp1
	ldi	Temp1, 0x04 	; Programmed by C. Hegarty to enable low voltage limit in multi mode
	st	X+, Temp1
...
;;;
;;;
;;; enunciate_value - beep out the value stored in I_Temp1 (between 1-99)
;;; A decimal number ab will be sounded out as 'a' beep_f1 followed by 'b' beep_f2
;;;  Assumptions: Interrupts must be disabled before calling this
;;;
;
;**** **** **** **** **** **** **** **** **** **** **** **** ****

enunciate_value:
	;; Ok to use I_Temp registers because interrupts disabled
loop10:
	cpi	I_Temp1, 10
	brlo	do1
	subi	I_Temp1, 10
...
;;;
;;;**** **** **** **** **** **** **** **** **** **** **** **** ****
;;; Measure throttle position (polling, not interrupt driven)
;;;
;;; Throttle position is measured in a busy loop with ~7us cycle time, number of high and low readings is counted
;;; for a total of 255 x 7 us = 1.8 ms
;;; This timing is chosen so that the sampling period is not close to a harmonic of the 15kHz PWM signal (to avoid beating)
;;;
;;; Number of low readings is returned in Temp3, number of high readings in Temp4
;;; Other registered used: XL, Temp1, Temp2
;;;**** **** **** **** **** **** **** **** **** **** **** **** ****
;;;

measure_throttle:
	ldi	Temp3, 0	; stores count of low readings
	ldi	Temp4, 0	; stores count of high readings
	ldi	Temp1, 255
input_check_1:
	Read_Rcp_Int XL		; Read input throttle signal
	sbrs 	XL, Rcp_In	; Is it high?
	inc	Temp3		; low reading
	sbrc	XL, Rcp_In	; Is it low?
	inc	Temp4		; high reading
	ldi	Temp2, 12
wait7us:			; Outer loop
	ldi	XH, 2
	dec	XH
...
;;; Delta Ray very slow to initialize - extended wait to 7s
	xcall wait1s
	xcall wait1s
	xcall wait1s
	xcall wait1s
...
init_no_signal:
	...
	sts	Lvc_Pulse_State, I_Temp1
	ldi	I_Temp1, 0	; lvc warning inactive
	...
	cpi	Temp4, 5
	brlo	bootloader_done
	;; if mostly high readings - run bootloader
	cpi	Temp4, 250
	brsh	run_bootloader
	;; otherwise do the throttle calibration
	ldi	Temp3, 255
...
bootloader_done:
	...
	lds	Temp3, Throttle_Calibrate
	cpi	Temp3, 255
	breq	decode		;If we are calibrating throttle no low check
...
skip_pwm_freq_measure:
	; Set up RC pulse interrupts
	Rcp_Int_First XH					; Enable interrupt and set to first edge
	Rcp_Clear_Int_Flag XH				; Clear interrupt flag
	cbr	Flags2, (1<<RCP_EDGE_NO)			; Set first edge flag

	lds	Temp1, Throttle_Calibrate
	cpi	Temp1, 255
	brne	beep_arm		; Normal start - otherwise enter throttle calibration loop
	sei
	xcall	wait200ms
	cli
	;; 	lds	I_Temp1, New_Rcp
	lds	I_Temp1, Throttle_Percentage
	;; 	lsr	I_Temp1		to set throttle range between 0 - 99% ;
	xcall	enunciate_value

	xcall	wait1s
	xcall	wait1s
	rjmp	skip_pwm_freq_measure
